...
class MyGLWidget : public QGLWidget 
{
  ...
  private:
    ...
    GLuint vertexLoc, normalLoc, matambLoc, matdiffLoc, matspecLoc, matshinLoc,matLoc;
    Model patr;
    // paràmetres calculats a partir de la capsa contenidora del model
    glm::vec3 centrePatr, trasllada;
    float escala;
    double radiEsc,ra,FOV,FOVinicial;
    glm::vec3 posFocus, colFocus;

    typedef  enum {NONE, ROTATE} InteractiveAction;
    InteractiveAction DoingInteractive;
    int xClick, yClick,material;
    float angleY, angleX, anglerot;
};
