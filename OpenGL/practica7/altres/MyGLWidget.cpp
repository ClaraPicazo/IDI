...
MyGLWidget::MyGLWidget (QGLFormat &f, QWidget* parent) : QGLWidget(f, parent), colFocus(1, 1, 0), posFocus(0, 0, 2)
{
...
void MyGLWidget::createBuffers ()
{
  ...
  glm::vec3 amb(0.0,0.0,1.0);
  glm::vec3 diff(0.0,0.0,1.0);
...
void MyGLWidget::carregaShaders ()
{
  ...
  matambLoc = glGetAttribLocation (program->programId(), "vmatamb");
  // Obtenim identificador per a l'atribut “matdiff” del vertex shader
  matdiffLoc = glGetAttribLocation (program->programId(), "vmatdiff");
  // Obtenim identificador per a l'atribut “matspec” del vertex shader
  matspecLoc = glGetAttribLocation (program->programId(), "vmatspec");
  // Obtenim identificador per a l'atribut “matshin” del vertex shader
  matshinLoc = glGetAttribLocation (program->programId(), "vmatshin");

  // Demanem identificadors per als uniforms del vertex shader
  transLoc = glGetUniformLocation (program->programId(), "TG");
  transpatrLoc = glGetUniformLocation (program->programId(), "TG_patr");
...
void MyGLWidget::modelTransformPatricio ()
{
  ...
  glUniformMatrix4fv (transpatrLoc, 1, GL_FALSE, &TG[0][0]);
  std::cout << escala << " " <<  posFocus.x << " " << posFocus.y << " " << posFocus.z << std::endl;
  lightFocus();
}
...
void MyGLWidget::viewTransform ()
{
  ...
  //View = glm::rotate(View, -float(M_PI)/4, glm::vec3(0.f, 0.f, 1.f));
  //View = glm::rotate(View, float(M_PI)/4, glm::vec3(1.f, 0.f, 0.f));
  View = glm::rotate(View, -angleY, glm::vec3(0, 1, 0));

  glUniformMatrix4fv (viewLoc, 1, GL_FALSE, &View[0][0]);
}

void MyGLWidget::lightFocus() {
...
void MyGLWidget::calculaCapsaModel ()
{
  ...
  posFocus = glm::vec3(centrePatr[0], centrePatr[1] + 12, centrePatr[2]);
}
